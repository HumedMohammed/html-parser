  ...
  return (
    <Card className="backdrop-blur-sm bg-white/90 dark:bg-gray-800/90 border-0 shadow-md h-fit">
      ...
      <CardContent className="h-[600px] overflow-y-scroll">
        {/* <ScrollArea className="h-[600px] pr-0 sm:pr-4"> */}
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="space-y-2">
                      <div className="text-xs text-muted-foreground">
                        <p className="truncate max-w-2/3">URL: {image.src}</p>
...
type Props = {
  image: ImageNode;
  handleImageUrlChange: (imageId: string, url: string) => void;
};
const QuickDialog = ({ image, handleImageUrlChange }: Props) => {
  const quickUrlController = useDisclosure();
  return (
    <Dialog
      open={quickUrlController.isOpen}
      onOpenChange={quickUrlController.onToggle}
    >
      ...
      <DialogContent className="sm:max-w-md">
        ...
        <div className="space-y-4">
            ...
            <Input
              ...
              onKeyDown={(e) => {
                if (e.key === "Enter") {
                  ...
                  if (input.value.trim()) {
                    handleImageUrlChange(image.id, input.value.trim());
                    // Close dialog
          ...
          <div className="flex gap-2">
            <Button
              variant="outline"
              onClick={quickUrlController.onClose}
              className="flex-1"
            >
              Cancel
            </Button>
...
type EditImageDialogProps = {
  handleEditImage: (image: ImageNode) => void;
  setNewImageUrl: React.Dispatch<React.SetStateAction<string>>;
  setNewAltText: React.Dispatch<React.SetStateAction<string>>;
  handleSaveImage: () => void;
  setEditingImage: React.Dispatch<React.SetStateAction<ImageNode | null>>;
  handleFileUpload: (imageId: string, file: File) => void;
  newImageUrl: string;
  newAltText: string;
  image: ImageNode;
};
const EditImageDialog = ({
  ...
  const editImageController = useDisclosure();
  return (
    <Dialog
      open={editImageController.isOpen}
      onOpenChange={editImageController.onToggle}
    >
      <DialogTrigger asChild>
        <Button
          variant="outline"
      ...
      <DialogContent className="sm:max-w-md">
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <p className="text-xs text-muted-foreground">
              Note: File upload functionality is coming soon. Use URL for now.
            </p>
          </div>
